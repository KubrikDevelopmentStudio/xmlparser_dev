...
$(document).ready(function () {
    ...
    $('#message_type_select').on('change', function () {
        var valid_param = $('#defaul_validation_param');
...
function clearLastResults() {
    ...
    $('#SECOND_DESCRIPTION_BLOCK').find('div.infos-msg').each(function (index, obj) {
        $(obj).remove();
    });
}
/**
...
function parseValidResults(data) {
    ...
    console.log(dataJSON[0].param_value);

    var block_body = $('#SECOND_DESCRIPTION_BLOCK');

    clearLastResults();

    var TAGS_COUNT_IN_XML          = dataJSON.all_tags_in_first_xml    ? dataJSON.all_tags_in_first_xml    : "";
    var TAGS_COUNT_IN_COMPARER_XML = dataJSON.all_tags_in_comparer_xml ? dataJSON.all_tags_in_comparer_xml : "";

    if(TAGS_COUNT_IN_XML != "" && TAGS_COUNT_IN_COMPARER_XML != "") {
            $(block_body).append("<div class='infos-msg'><br>Всего тегов в эталонной XML: " + TAGS_COUNT_IN_XML +
                                 "<br>Всего тегов в проверяемой XML: " + TAGS_COUNT_IN_COMPARER_XML + "<br><br></div>");
    }
    ...
    $(block_body).append("<div class='infos-msg'>Всего фатальных ошибок: " + fatal_error_count + "</div>");
    $(block_body).append("<div class='infos-msg'>Всего ошибок: " + error_count + "</div>");
    $(block_body).append("<div class='infos-msg'>Всего предупреждений: " + warnings_count + "</div>");
    
    $(block_body).append("<div class='infos-msg'><br>Всего событий: " + all_events + "</div>");
}
